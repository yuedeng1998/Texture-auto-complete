...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	for r in range(ssd_rows):
		for c in range(ssd_cols):
			...
			#
			sump = 0
			neibers = np.where(TODOMask == 0)
			# import pdb; pdb.set_trace()
			x = neibers[0]
			y = neibers[1]

			# image: width x height x 3
			currentTexture = textureIm[x+r, y+c,:]
			# number of points x 3, the pixel value in texture = pixel in patch + offset
			currentPatch =  TODOPatch[x, y,:]# number of points x 3
			sump = (currentPatch * 1.0 - currentTexture *1.0)
			sump = np.square(sump)
			sump = np.sum(sump)
			SSD[r,c] = sump
	return SSD
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	for i in range(patchSize):
		for j in range(patchSize):
			...
			#
			if(TODOMask[i,j] == 1):
				#the position of each corresponding pixel is different for imHole & textureIm
